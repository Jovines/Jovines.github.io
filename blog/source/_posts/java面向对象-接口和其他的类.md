...
> ```
 ...
 到这里了我们需要明确一点，**内部类是个编译时的概念**，一旦编译成功后，它就与外围类**属于两个完全不同的类**（当然他们之间还是有联系的）。对于一个名为OuterClass的外围类和一个名为InnerClass的内部类，在编译成功后，会出现这样两个class文件：**OuterClass.class和OuterClass$InnerClass.class。** 
